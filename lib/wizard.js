...
function chooseOptions() {
  ...
  inquirer.prompt({
    ...
    globalObj.options = {
      regex: false,
      force: _.includes(answers.options, 'f'),
      verbose: _.includes(answers.options, 'v'),
      noIndex: _.includes(answers.options, 'n'),
      noTrim: false
    };
    if (_.includes(answers.options, 'r')) {
      clear();
      inquirer.prompt({
        type: 'input',
        name: 'regex',
...
function getCommandText() {
  ...
  let filesArr = [];
  _.forEach(globalObj.inputFiles, function(value) {
    filesArr.push(escapeText(value));
  });
